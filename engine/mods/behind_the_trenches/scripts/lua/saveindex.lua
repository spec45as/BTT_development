...
function SaveIndex:GetSaveBackupName()
	...
	name = name .. "_backup_" .. os.time(os.date("!*t"))
	return name
end

function SaveIndex:SaveBackup(callback)
	self:Save(callback, self:GetSaveBackupName(), true)
end
...
function SaveIndex:SetSaveVolcanoData()
	...
	if vm then
		vmdata = {}
		vmdata["appeasesegs"] = vm.appeasesegs
		vmdata["firerain_timer"] = vm.firerain_timer
		vmdata["firerain_sps"] = vm.firerain_spawn_per_sec
		vmdata["smoke_timer"] = vm.smoke_timer
		vmdata["ash_timer"] = vm.ash_timer
		vmdata["waserupting"] = vm.waserupting
	end
...
function SaveIndex:LoadSavedVolcanoData()
	...
	if vm and vmdata then
		vm.appeasesegs = vmdata["appeasesegs"]
		vm.firerain_timer = vmdata["firerain_timer"]
		vm.firerain_spawn_per_sec = vmdata["firerain_sps"]
		vm.smoke_timer = vmdata["smoke_timer"]
		vm.ash_timer = vmdata["ash_timer"]
		vm.waserupting = vmdata["waserupting"]
	end
...
function SaveIndex:DeleteSlot(slot, cb, save_options)
	...
	if slot_exists then
		...
		if save_options == true then
			...
			self.data.slots[slot].modes["survival"] = {survivaloptions = survivaloptions}
			self.data.slots[slot].modes["shipwrecked"] = {shipwreckedoptions = shipwreckedoptions}
		end
		self:Save(cb)		
	elseif cb then
		cb()
...
function SaveIndex:BackupSlot(slotnum, onbackedup)

	local current_mode = self.data.slots[slotnum].current_mode
	local savename = self:GetSaveGameName(current_mode, slotnum) .. "_bckp_" .. os.time(os.date("!*t"))
...
function SaveIndex:MergeSlotWithCurrent(slotnum)
	...
	local function PrepareTargetForMerge(savedata)
		-- TODO: set to mild if the target is Shipwrecked
		...
		--table.insert(player_inventory, savedata.playerinfo.data.inventory.equip)
		...
		-- dumptable(player_inventory)

		local new_gravestone = { data={ mounddata={ prefab="inventorymound", x=player_x+0.35, z=player_z-0.35, 
			buriedinventory= player_inventory,
			} 
		}, x=player_x, z=player_z}

		local inventorygrave = {}
		table.insert(inventorygrave, new_gravestone)
		savedata.ents.inventorygrave = inventorygrave
...
--	local data = self:GetModeData(saveslot, "survival")
	...
	print("SaveIndex:StartSurvivalMode!:", slot.dlc.REIGN_OF_GIANTS, slot.dlc.CapyDLC)
	...
	if slot.dlc.CapyDLC and slot.dlc.CapyDLC == true then
...
function SaveIndex:LeaveShipwrecked(onsavedcb)
	local playerdata = {}
    local player = GetPlayer()
    if player then
        playerdata = player:GetSaveRecord().data
...
function SaveIndex:MergeShipwrecked(slotnum, onsavedcb)
	if not slotnum then
		return
	end
	...
	if not self:OwnsMode("survival") then
		
		-- We backup before doing anything, don't wanna have to deal with angry players..
		self:BackupSlot(self.current_slot, slotbackupdone)
		self:BackupSlot(slotnum, slotbackupdone)
		self:SaveBackup(indexbackupdone)

		-- Copy the survival data
		self.data.slots[self.current_slot].modes.survival = deepcopy(self.data.slots[slotnum].modes.survival)

		local newfile = self:MergeSlotWithCurrent(slotnum)
		local oldfile = self.data.slots[self.current_slot].modes.survival.file

		self.data.slots[self.current_slot].modes.survival.file = newfile
		for i=1,#self.data.slots[self.current_slot].modes.survival.files do
			if self.data.slots[self.current_slot].modes.survival.files[i] == oldfile then
				self.data.slots[self.current_slot].modes.survival.files[i] = newfile
			end
		end
	end
...
function SaveIndex:EnterShipwrecked(onsavedcb, saveslot)
	self.current_slot = saveslot or self.current_slot

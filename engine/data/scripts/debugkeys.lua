...
AddGameDebugKey(KEY_F5, function()
	if TheInput:IsKeyDown(KEY_SHIFT) then
		...
		local x, y = ground.Map:GetTileCoordsAtPoint(pos.x, pos.y, pos.z)
		local minimap = TheSim:FindFirstEntityWithTag("minimap")
			if TheInput:IsKeyDown(KEY_CTRL) then
				local maxX, maxY = WorldSim:GetWorldSize() 
				for x = 0, maxX do
					for y = 0, maxY do
						local tile = ground.Map:GetTile(x,y)
						if tile == GROUND.IMPASSABLE or tile == GROUND.INVALID then
							local newTile = GROUND.FOREST
							if math.random() > 0.5 then
								newTile = GROUND.GRASS
							end
							ground.Map:SetTile( x, y, newTile )
							ground.Map:RebuildLayer( newTile, x, y )

						end
					end
				end
				if minimap then
					minimap.MiniMap:RebuildLayer( GROUND.IMPASSABLE, 1, 1 )
				end
			else							
				print(ground.Map:GetTileAtPoint(x,0,y))
				print(x,y)
...
AddGameDebugKey(KEY_F11, function()
    ...
    for i = 1, #positions do
        ...
        GetPlayer():DoTaskInTime(delay, function() 
            local prefab = SpawnPrefab("pigguard")

...
local NewGameScreen = Class(Screen, function(self, slotnum)
	...
	--   }
		...
		table.insert(menuitems, {text = STRINGS.UI.NEWGAMESCREEN.CANCEL, cb = function() TheFrontEnd:PopScreen(self) end, offset = Vector3(0, yIncrement, 0)})
  	else
  		menuitems = 
	    {
...
function NewGameScreen:Start()
	...
	local function StartGame(mode)
		...
		TheFrontEnd:Fade(false, 1, function()
			SaveGameIndex:StartSurvivalMode(self.saveslot, self.character, self.customoptions, onsaved, GetEnabledDLCs(), mode)
		end )
	end
	...
	elseif IsDLCInstalled(CAPY_DLC) and self.CapyDLC then
		...
		-- TheFrontEnd:PushScreen(BigPopupDialogScreen("Shipwrecked", "Start in Shipwrecked or Volcano?",
		-- 	{{text="Shipwrecked",
		-- 		cb = function()
		-- 			TheFrontEnd:PopScreen()
		-- 			StartGame("shipwrecked")
...
function NewGameScreen:MakeCapyButton()
	...
	self.Capybutton = self:AddChild(Widget("option"))
